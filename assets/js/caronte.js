...
Genie.WebChannels.clientid = crypto.randomUUID()
...
Genie.WebChannels.load_channels = function() {
  ...
  Genie.WebChannels.connect = connect;
...
//  }
  ...
  function sendMessageTo(topic, payload = {}) {
    if (Genie.WebChannels.socket.readyState === 1) {
  ...
  function connect(id, setting = {interest:"*"}) {
    if (Genie.WebChannels.socket.readyState === 1) {
      Genie.WebChannels.socket.send(JSON.stringify({
        'id': crypto.randomUUID(),
        'identity': id,
...
function subscribe() {
  if (document.readyState === "complete" || document.readyState === "interactive") {
    ...
    Genie.WebChannels.connect(Genie.WebChannels.clientid);
